...
Профили вложенные: каждый следующий включает предыдущие, поэтому выбор один, а не набор галочек.
Docling распознаёт буквы тем же Vision, а лёгкий режим Vision без Docling нужен как быстрый вариант
для простых сканов.

| Профиль | Что ставится | Место на диске | Пункты в меню Finder |
|---|---|---|---|
| `1` base | только движок AnyDoc; сканы пропускаются с пометкой «нужен OCR» | ~30 МБ | «Конвертировать в Markdown (без OCR)» |
| `2` vision | base + OCR встроенным в macOS Vision framework | ~50 МБ | «… (без OCR)», «… (OCR: Vision)» |
| `3` docling | vision + [Docling](https://github.com/docling-project/docling): заголовки, таблицы, порядок чтения на сканах | ~1,2 ГБ + модели 0,7 ГБ | «… (без OCR)», «… (OCR: Vision)», «… (OCR: Docling)» |

Все пункты работают одновременно: под каждую папку выбираешь нужный прямо в Finder.
